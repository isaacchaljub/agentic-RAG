...
# =============================================================================
# Agentic RAG — FastAPI service (v2 / LangGraph), CPU-only.
#
# Multi-stage build:
...
# official uv image. Pinning the version (0.9.27) keeps builds reproducible.
...
# compromised, the blast radius is a nobody-user, not root. (Interview classic.)
RUN groupadd --system app \
 && useradd --system --gid app --home-dir /app --create-home app

# Runtime environment:
#   PATH               → put the venv first so `python`/`fastapi` = venv's.
#   PYTHONUNBUFFERED   → stream logs immediately (don't buffer) → visible in
#                        `docker logs` / CloudWatch / Azure log stream.
...
# Copy ONLY the code this API needs. No root __init__.py (project root is not a
# package), so `serving_api` and `pipeline` are top-level packages and the
# absolute import `from pipeline...` resolves. `documents/` is read at startup
# to build the FAISS index (see main_v2.py: parent.parent / 'documents').
...
# Container-level health probe. Orchestrators (ECS, App Runner, Container Apps)
# use this to know when the app is actually ready vs. just "process started".
#   start-period=120s → grace window while it loads the model + builds the index.
# /health returns 200 only once the vector DB is ready; a 500 raises → unhealthy.
...
# Equivalent: uvicorn serving_api.main_v2:app --host 0.0.0.0 --port 8000
CMD ["fastapi", "run", "serving_api/main_v2.py", "--host", "0.0.0.0", "--port", "8000"]
